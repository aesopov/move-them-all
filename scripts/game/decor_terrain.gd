...
@export var theme := "jungle":
	set(v):
		theme = v
		queue_redraw()
@export_enum("slab_1x05", "slab_2x05", "corner_cut", "fallen_log", "broken_column", "rubble", "barricade", "broken_arch", "mechanism", "relic_tall", "cargo_long", "relic_small") var asset := "slab_1x05":
	set(v):
		asset = v
		queue_redraw()
...
func _draw() -> void:
	...
	draw_set_transform(extent * 0.5, quarter_turns * PI * 0.5)
