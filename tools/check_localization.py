...
assert rows[0] == ['keys', 'en', 'es', 'pt_BR', 'fr', 'de', 'zh_CN', 'ru']
keys = set()
for row in rows[1:]:
    assert len(row) == 8 and all(row), row
...
print(f'{len(keys)} messages × 7 languages; placeholders and level titles verified')
