extends SceneTree
## Verify and render round three through the actual scenery renderer.
...
func run() -> void:
	await process_frame
	var manifest: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://prompts/world_terrain_kits_3.json"))
	...
	canvas.add_child(bg)
	var titles := ["Tall relic · 1×2", "Long cargo · 2×1", "Small relic · 1×1"]
	for j in 3:
	...
	canvas.get_texture().get_image().save_png("res://assets/reference/terrain_kit/worlds_round3_preview.png")
	print("Round three terrain checks: %d failures, %d assets" % [failures, assets.size()])
